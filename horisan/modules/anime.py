...
def whatanime(_, message):
    ...
    if reply and reply.media:
        ...
        anilist_id = info.result[0].anilist.id
        ...
        reply.reply_document(f"{reply.from_user.id}.mp4", caption=data, reply_markup=keyboard)
...
async def waifu(client, message):
    ...
    level = mongo_client["WaifuDb"]["waifu"] 
    if message.chat.type is not "ChatType.PRIVATE":
        if message.from_user is not None:
            ...
            if xpnum is None:
                ...
                xp -= ((50*((l-1)**2))+(50*(l-1)))
                if xp == 0:
                    query = random.choice(WAIFUS_PIC)
                    try:
                        zerochan.search(query)
                        zerochan.size(PictureSize.BIGGER_AND_BETTER)
                        zerochan.sort(SortBy.POPULAR)
                        zerochan.page(1)
                        data = zerochan.pics()
                        img = data.images[0]
                        image = img.url
                    ...
                    if json:
                        char_name = f"{json.get('name').get('full')}"
                        await pgram.send_photo(message.chat.id, image, caption= f"**A waifu appeared!**\nAdd them to your harem by sending /protecc character name")
                        REDIS.sadd(f"waifu_list{message.chat.id}", char_name)

def protecc(update, context):
    message = update.effective_message
    user = update.effective_user
    waifu_list = list(REDIS.sunion(f"waifu_list{update.effective_chat.id}"))
    if not waifu_list:
        return
    search = message.text.split(" ", 1)
    search = search[1]
    ...
    if json:
        char_name = f"{json.get('name').get('full')}"
        if char_name in waifu_list:
            REDIS.sadd(f"anime_waifu{user.id}_{update.effective_chat.id}", char_name)
            update.effective_message.reply_text(f"OwO you protecc'd {char_name}. This waifu has been added to your harem.")
            REDIS.srem(f"waifu_list{update.effective_chat.id}", char_name)
        else:
...
def fvrt_waifu(update, context):
    ...
    buttons = [
                [
                    InlineKeyboardButton(
                        "Inline 🌐", switch_inline_query_current_chat=f"harem {update.effective_user.id}"
                    )
                ]
            ]
    if update.effective_chat.type == "private":
        message.reply_text("This Command Will Works In Groups")
    else:
        fvrt_char = list(REDIS.sunion(f"anime_waifu{user.id}_{update.effective_chat.id}"))
        if fvrt_char: 
            fvrt_char.sort()
            waiphus = "\n• ".join(fvrt_char)
            zerochan.search(fvrt_char[0])
            zerochan.size(PictureSize.BIGGER_AND_BETTER)
            zerochan.sort(SortBy.POPULAR)
            zerochan.page(1)
            data = zerochan.pics()
            ...
            document=img.url,
            caption=f"{user.username}'s harem in {update.effective_chat.title}:\n • {waiphus}",
            reply_markup=InlineKeyboardMarkup(buttons),
            )
        else:
