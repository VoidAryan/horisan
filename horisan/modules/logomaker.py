...
async def logo_gen(event):
    xx = await event.reply("`Hori San is Creating your logo waito...`")
    ...
    if event.reply_to_msg_id:
        ...
        async for i in ubot2.iter_messages(
            "@hdcoolanime", filter=InputMessagesFilterPhotos
        ):
            pics.append(i)
    ...
    if os.path.exists(flnme):
        await tbot.send_file(
            event.chat_id,
            file=flnme,
            caption="Logo by [Hori San](https://t.me/kyoukohori_robot)",
